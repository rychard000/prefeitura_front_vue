...
export default {
  ...
  beforeMount() {
    EstruturaOrganizacional
      ...
      .then((response) => {
        console.log(response.pessoa_juridica)
        this.estrutura = response
        this.pessoa_juridica = response.pessoa_juridica
        this.competencias = response.pessoa_juridica.competencias
      })
  }
}
</script>
  ...
  <section id="sec" class=" m-5  d-flex align-items-center justify-content-center">
    ...
    <section class="d-flex gap-5">
      ...
      <article :style="{
        ...
        <small>{{pessoa_juridica.cargo.nome}}</small>
